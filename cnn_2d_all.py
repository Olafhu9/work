...
resultDir = "/home/juhee5819/T2+/result/1206/2d_allvar"
...
var_event = ['nbjets_m', 'ngoodjets', 'Ht']
# jet variables
...
var_all = var_event + var_jet + var_lepton
...
train_data_input = for_train.filter( items = var_all )

valid_data_out = for_valid.filter( items = ['category'] )
valid_data_input = for_valid.filter( items = var_all )
...
train_data_input = train_data_input.reshape( train_data_input.shape[0], 6, -1, 1 )

valid_data_out = valid_data_out.reshape( valid_data_out.shape[0], 1, valid_data_out.shape[1] )
valid_data_input = valid_data_input.reshape( valid_data_input.shape[0], 6, -1 , 1)

# Inputs
Input_data = Input( shape = (train_data_input.shape[1], train_data_input.shape[2], train_data_input.shape[3]) )

# BatchNormalization
x = BatchNormalization( name = 'data_input_batchnorm' )(Input_data)

# CNN for jets
x = Conv2D(32, (2,2), padding='same', kernel_initializer='lecun_uniform',  activation='relu', name='x_conv0')(x)
x = Conv2D(32, (2,2), padding='same',  kernel_initializer='lecun_uniform',  activation='relu', name='x_conv1')(x)
x = Reshape( (1, -1) )(x)
x = LSTM(50, go_backwards=True, implementation=2, name='x_lstm', return_sequences=True)(x)
#x = LSTM(50, go_backwards=True, implementation=2, name='x_lstm2', return_sequences=True)(x)
...
model = Model( inputs = Input_data, outputs = cat_pred)

batch_size = 1024
epochs = 10

model.compile( loss = 'categorical_crossentropy',optimizer = 'adam',metrics=['accuracy', 'categorical_accuracy'] )
hist = model.fit( x = [train_data_input], y = train_data_out, batch_size = batch_size, epochs = epochs, validation_data = ( valid_data_input, valid_data_out))

#pre = model.predict(valid_data)
pred = model.predict( [valid_data_input])
